#!/bin/bash
# Feb, 2024
# Check if the subjects went through xcp_d withOUT --cifti flag successfully
# ABCD study, Ashmeet Jolly

site="P064"
# Define the directory where the subject folders are located
subjects_dir="/scratch/project_2001640/ABCD_fMRI/${site}/BIDS/derivatives/xcpd_noncifti"

# Create a variable to store the path of the non-faulty MRIQC file
non_faulty_xcpd_noncifti_file="/scratch/project_2001640/ABCD_fMRI/csc_fmri/scripts/downcmd_txt_files/${site}_non_faulty_xcpd_noncifti.txt"

# Create a variable to store the path of the faulty MRIQC file
faulty_xcpd_noncifti_file="/scratch/project_2001640/ABCD_fMRI/csc_fmri/scripts/downcmd_txt_files/${site}_faulty_xcpd_noncifti.txt"

# Check if the directories exist
if [ ! -d "$(dirname "$non_faulty_xcpd_noncifti_file")" ]; then
    echo "Directory $(dirname "$non_faulty_xcpd_noncifti_file") does not exist."
fi

if [ ! -d "$(dirname "$faulty_xcpd_noncifti_file")" ]; then
    echo "Directory $(dirname "$faulty_xcpd_noncifti_file") does not exist."
fi
...
# Exclude 'atlases' directory
        ...
        if [[ -d "$subjects_dir/$subject_id" && -f "$subjects_dir/${subject_id}.html" ]]; then
           # Check if the additional conditions are met
            if [ -d "$subjects_dir/$subject_id/ses-baselineYear1Arm1/anat" ]; then
                ...
                if [ "$num_files_anat" -eq 38 ]; then
                    ...
                    if [ -d "$subjects_dir/$subject_id/ses-baselineYear1Arm1/func" ]; then
                        ...
                        if [ "$num_files_func" -eq 155 ]; then
                            # All conditions are met, append this subject to the non-faulty subjects file
                            echo "All conditions met for $subject_id"
                            echo $subject_id >> "$non_faulty_xcpd_noncifti_file"
                        else
                            echo "Number of files in func directory is not 155 for $subject_id"
                            all_conditions_met=false
                            echo $subject_id >> "$faulty_xcpd_noncifti_file"  # Add subject ID to faulty file
                        fi
                    else
            ...
            echo $subject_id >> "$faulty_xcpd_noncifti_file"
            # Set the flag to false
            all_conditions_met=false
        fi
###removal of faulty####
        # Delete the subject directory and associated files if listed in the faulty file
        if grep -q "$subject_id" "$faulty_xcpd_noncifti_file"; then
...
sed -i 's/sub-NDAR//' "$non_faulty_xcpd_noncifti_file"
sed -i 's/sub-NDAR//' "$faulty_xcpd_noncifti_file"


# Filter out duplicate subject IDs from non-faulty and faulty files
awk '!seen[$1]++' "$non_faulty_xcpd_noncifti_file" > temp_non_faulty.txt
mv temp_non_faulty.txt "$non_faulty_xcpd_noncifti_file"

awk '!seen[$1]++' "$faulty_xcpd_noncifti_file" > temp_faulty.txt
mv temp_faulty.txt "$faulty_xcpd_noncifti_file"
